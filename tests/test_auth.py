...
@pytest.mark.asyncio
async def test_authenticate(create_user: models.User, async_session: AsyncSession):
    user = await auth.authenticate_user("test@example.com", "string", async_session)
    assert user
...
@pytest.mark.asyncio
async def test_get_current_user(create_user: models.User, async_session: AsyncSession):
