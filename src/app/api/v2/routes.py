...
async def create_user(user: schemas.UserCreate, db: AsyncSession = Depends(get_db)):
    ...
    if db_user:
        raise HTTPException(status_code=400, detail={'status': 400, 'message': "Email already registered"})
...
async def read_all_passages(skip: int = 0, limit: int = 100, db: AsyncSession = Depends(get_db)):
