...
async def read_all_users(
    ...
    return await crud.get_all_objects(
        db, model=models.User, skip=skip, limit=limit
    )
...
async def read_user_by_id(user_id: int, db: AsyncSession = Depends(get_db)):
    db_user = await crud.get_object_by_id(
        db, model=models.User, obj_id=user_id
    )
    if db_user is None:
        raise HTTPException(
...
@v2_app.get(
    '/passages/', response_model=list[schemas.Passage], tags=['Passes']
)
async def read_all_passages(
    skip: int = 0, limit: int = 100, db: AsyncSession = Depends(get_db)
):
    return await crud.get_all_objects(
        db, model=models.Passage, skip=skip, limit=limit
    )


@v2_app.get(
    '/passages/{passage_id}', response_model=schemas.Passage, tags=['Passes']
)
